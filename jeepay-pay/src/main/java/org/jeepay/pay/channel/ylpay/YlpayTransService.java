...
@Service
public class YlpayTransService extends BaseTrans {
    ...
    private static final MyLog _log = MyLog.getLog(YlpayTransService.class);
    ...
    public JSONObject doTransPay(TransOrder transOrder) {
        ...
        try {
            ...
            YlpayConfig ylpayConfig = new YlpayConfig(getTransParam(transOrder));
    ...
    public JSONObject balance(String payParam) {
        ...
        JSONObject retObj = buildRetObj();
        try{
            YlpayConfig ylpayConfig = new YlpayConfig(payParam);
            MsgBean req_bean = new MsgBean();
            req_bean.setVERSION("2.1");
