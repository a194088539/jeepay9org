...
<script>
    ...
    function paymentOrder(amount){
        $.ajax({
            url: '/api/payment/scan_pay?mchId=${mchId!}&appId=${appId!}&codeId=${codeId!}&channelId=${channelId!}&passageId=${passageId!}&openId=${openId!}&amount=' + amount,
            type: 'GET',
            cache: false,
            async: false,
            dataType: 'JSON',
