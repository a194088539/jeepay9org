package org.jeepay.agent.order.ctrl;
...
public class PayOrderController extends BaseController {
    ...
    public ResponseEntity<?> list(HttpServletRequest request) {
        ...
        List<PayOrder> payOrderList = rpcCommonService.rpcPayOrderService.selectByAgentList(
                getUser().getId(),(getPageIndex(page) -1) * getPageSize(limit), getPageSize(limit), payOrder, createTimeStart, createTimeEnd);
    ...
    public ResponseEntity<?> count(HttpServletRequest request) {
        ...
        String createTimeStartStr = getString(param, "createTimeStart");
        String createTimeEndStr = getString(param, "createTimeEnd");
        Map allMap = rpcCommonService.rpcPayOrderService.count4All(angentId,null,null, mchId, productId, payOrderId, mchOrderNo, productType, createTimeStartStr, createTimeEndStr);
        Map successMap = rpcCommonService.rpcPayOrderService.count4Success(angentId,null,null, mchId, productId, payOrderId, mchOrderNo, productType, createTimeStartStr, createTimeEndStr);
        Map failMap = rpcCommonService.rpcPayOrderService.count4Fail(angentId, null, null,mchId, productId, payOrderId, mchOrderNo, productType, createTimeStartStr, createTimeEndStr);
