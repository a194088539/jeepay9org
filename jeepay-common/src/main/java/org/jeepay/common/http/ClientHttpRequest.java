...
import org.jeepay.common.util.XML;
...
import static org.jeepay.common.http.UriVariables.getMapToParameters;
...
public class ClientHttpRequest<T> extends HttpEntityEnclosingRequestBase implements  org.apache.http.client.ResponseHandler<T>{
     ...
     *
     * @param method 请求方式
     * {@link org.jeepay.common.bean.MethodType} 请求方式
     */
    ...
    private T toBean(HttpEntity entity, String[] contentType) throws IOException {
        ...
        if (isText(contentType[0])) {
            ...
            if (null != contentType && contentType.length == 2) {
            ...
            if (isXml(contentType[0], first)) {
                ...
                return XML.toJSONObject(result, charset).toJavaObject(responseType);
            }

            // text/plain
            if (isTextPlain(contentType[0])) {
    ...
    private boolean isTextPlain(String contentType) {
        return (ContentType.TEXT_PLAIN.getMimeType().equals(contentType));
    }
