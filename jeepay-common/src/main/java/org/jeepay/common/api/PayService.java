...
public interface PayService<PC extends PayConfigStorage> {
     ...
     * @return 返回图片信息，支付时需要的
     */
    String genQrPayContent(PayOrder order);
     ...
     * @return 返回查询回来的结果集，支付方原值返回
     */
    Map<String, Object> query(Map<String, Object> queryMap);
    /**
     * 交易查询接口，带处理器
     * @param tradeNo    支付平台订单号
     * @param outTradeNo 商户单号
     ...
     * @return 对应的转账订单
     */
    Map<String, Object> transferQuery(Map<String, Object> queryParams);
     ...
     * @param handler 消息处理器
     * 配合{@link  org.jeepay.common.api.PayService#payBack(Map, InputStream)}进行使用
     *
     *   默认使用{@link  org.jeepay.common.api.DefaultPayMessageHandler }进行实现
     *
     */
